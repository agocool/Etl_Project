...
class mod3saTest(unittest.TestCase):
    ...
    def logset_temp(log):
        logr = mod3saTest.setup_logger('temp', '/home/talentum/zomato_etl/logs/module1.log')
        logr.info(log)
        logging.shutdown()

    def logset(log, log_name):
        file_name = '/home/talentum/zomato_etl/logs/' + str(log_name) + ".log"
        logr = mod3saTest.setup_logger('main', file_name)
        logr.info(log)
    def test_mod3sa(self):

        st_time = datetime.now()
        start_time = st_time.strftime('%Y-%m-%d %H:%M:%S')
        spark = SparkSession.builder.appName("Spark Module 3").enableHiveSupport().getOrCreate()
        ...
        try:
            ...
            print()
            expected_value = 11595
            value = df.count()
            self.assertEqual(value, expected_value)
            # spark.sql("drop table a_gokul.zomato_summary")
            df.write.format("orc").mode("overwrite").partitionBy("p_filedate", "p_country_name").saveAsTable(
                "a_gokul.zomato_sum_temp")
        ...
        log = job_id + " " + job_step + " " + str(start_time) + " " + str(end_time) + " " + job_status
        mod3saTest.logset_temp(log)
        mod3saTest.logset(log, final_log)
